...
class ItemService
{
    ...
    public function updateItem(string $idItem, array $user, array $payload, array $categoryFields): array
    {
        $existing = $this->items->findOwnedItem($idItem, $user['id_user']);
        if (!$existing) {
            JsonResponse::error('You are not allowed to edit this item.', 403);
        }

        // Reuse validation/mapping from create, but keep existing slug
        $errors = Validator::required($payload, ['title', 'description', 'id_category', 'listing_type', 'item_status']);
        if ($errors) {
            JsonResponse::error('Validation failed.', 422, $errors);
        }
        ...
        return $this->items->updateItemWithFields($idItem, $itemData, $fieldValues);
    }
}
